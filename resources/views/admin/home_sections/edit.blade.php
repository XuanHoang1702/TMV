...
@section('title', 'Chỉnh sửa Home Section')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1>Chỉnh sửa Home Section</h1>
    ...
    <div class="card-body">
        <form action="{{ route('admin.home_sections.update', $homeSection) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $homeSection->title) }}" required>
                @error('title')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="4">{{ old('content', $homeSection->content) }}</textarea>
                @error('content')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <div id="images-container">
                    @if($homeSection->images && is_array($homeSection->images))
                        @foreach($homeSection->images as $index => $image)
                            <div class="image-item mb-3 border p-3">
                                <h6>Image {{ $index + 1 }}</h6>
                                @if($image)
                                    <div class="mb-2">
                                        <img src="{{ Storage::url($image) }}" alt="Current Image" style="width: 100px;">
                                        <small class="text-muted">Current image. Upload new to replace.</small>
                                    </div>
                                @endif
                                <input type="file" class="form-control @error('images.' . $index) is-invalid @enderror" name="images[]" accept="image/*">
                                @error('images.' . $index)
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
            ...
            <div class="mb-3">
                ...
                <div id="list-items-container">
                    @if($homeSection->list_items && is_array($homeSection->list_items))
                        @foreach($homeSection->list_items as $index => $item)
                            <div class="list-item mb-3 border p-3">
                                <h6>List Item {{ $index + 1 }}</h6>
                                <div class="mb-2">
                                    <label class="form-label">Icon</label>
                                    @if(isset($item['icon']) && $item['icon'])
                                        <div class="mb-2">
                                            <img src="{{ Storage::url($item['icon']) }}" alt="Current Icon" style="width: 50px;">
                                            <small class="text-muted">Current icon. Upload new to replace.</small>
                                        </div>
                                    @endif
                                    <input type="file" class="form-control @error('list_icons.' . $index) is-invalid @enderror" name="list_icons[]" accept="image/*">
                                    @error('list_icons.' . $index)
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="mb-2">
                                    <label class="form-label">Title</label>
                                    <input type="text" class="form-control @error('list_titles.' . $index) is-invalid @enderror" name="list_titles[]" value="{{ old('list_titles.' . $index, $item['title'] ?? '') }}">
                                    @error('list_titles.' . $index)
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="mb-2">
                                    <label class="form-label">Description</label>
                                    <textarea class="form-control @error('list_descriptions.' . $index) is-invalid @enderror" name="list_descriptions[]" rows="3">{{ old('list_descriptions.' . $index, $item['description'] ?? '') }}</textarea>
                                    @error('list_descriptions.' . $index)
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-control @error('position') is-invalid @enderror" id="position" name="position" required>
                    ...
                    <option value="1" {{ old('position', $homeSection->position) == '1' ? 'selected' : '' }}>Section 1</option>
                    <option value="2" {{ old('position', $homeSection->position) == '2' ? 'selected' : '' }}>Section 2</option>
                </select>
                @error('position')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', $homeSection->order) }}" min="0">
                @error('order')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $homeSection->is_active) ? 'checked' : '' }}>
