                    ...
                    <div class="form-group">
                        ...
                        <div class="related-news-container" id="related-news-container">
                            @php
                                ...
                                $relatedNews = \App\Models\News::whereIn('id', $selectedRelated)->where('is_active', true)->whereNotNull('published_at')->get();
                            @endphp
                            @foreach ($relatedNews as $related)
                                <div class="related-news-item" data-id="{{ $related->id }}">
                                    @if ($related->images && count($related->images) > 0)
                                        <img src="{{ Storage::url($related->images[0]) }}" alt="{{ $related->title }}">
                                    @else
                        ...
                        <input type="hidden" name="related_news[]" id="related-news-input" value="{{ json_encode($selectedRelated) }}">
                        <button type="button" class="btn add-related-btn mt-2" data-toggle="modal" data-target="#relatedNewsModal">
                            <i class="fas fa-plus me-1"></i>Thêm bài viết liên quan
                        </button>
                        <small class="form-text text-muted">Kéo và thả để sắp xếp thứ tự. Nếu không chọn, hệ thống sẽ tự động hiển thị bài viết cùng chuyên mục.</small>
                    </div>
                        ...
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="relatedNewsModalLabel"><i class="fas fa-newspaper me-2"></i>Chọn bài viết liên quan</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <input type="text" class="search-input" id="related-news-search" placeholder="Tìm kiếm bài viết...">
                                    <div id="news-options">
                                        @foreach (\App\Models\News::where('is_active', true)->whereNotNull('published_at')->where('id', '!=', $news->id)->orderBy('published_at', 'desc')->get() as $option)
                                            <div class="news-option" data-id="{{ $option->id }}">
                                                @if ($option->images && count($option->images) > 0)
                                                    <img src="{{ Storage::url($option->images[0]) }}" alt="{{ $option->title }}">
                                                @else
                                                    <img src="https://via.placeholder.com/40" alt="No image">
                                                @endif
                                                <div>
                                                    <div class="title">{{ $option->title }}</div>
                                                    <div class="date">{{ $option->published_at->format('d/m/Y') }}</div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                @foreach ($news->images as $image)
                                    <div class="col-md-3 mb-2">
                                        <div class="border p-2 position-relative">
                                            <img src="{{ Storage::url($image) }}" class="img-fluid" style="max-width:100px">
                                            <button type="button" class="btn btn-danger btn-sm remove-existing-image mt-1 position-absolute"
                                                    style="top: 5px; right: 5px; z-index: 10;" data-image-path="{{ $image }}"
...
$(document).ready(function() {
    ...
    $('#relatedNewsModal').on('show.bs.modal', function() {
        ...
        options.forEach(option => {
            option.style.display = selectedIds.includes(option.dataset.id) ? 'none' : 'flex';
        });
    });
