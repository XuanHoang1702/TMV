        ...
        <tbody>
            @foreach($siteInfo as $info)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('admin.siteInfo.edit', $info->id) }}" class="btn btn-warning btn-sm">Sửa</a>
                        <form action="{{ route('admin.siteInfo.destroy', $info->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
